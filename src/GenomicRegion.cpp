...
GenomicRegion::GenomicRegion(const string s) {
  ...
  if ( (s.find('(') != string::npos) && (s.find(')') != string::npos) ) { // it s a (-) string
    ...
    // scrub commas
    string s_s = SnowUtils::scrubString(s, ",");

    string schr, spos1, spos2, sstrand;
    stringstream iss(s_s);
    getline(iss, schr, ':');
    getline(iss, spos1, '-');
    getline(iss, spos2, '(');
    getline(iss, sstrand, ')');
    ...
    try {
      chr = chrToNumber(schr);
      pos1 = stoi(spos1);
      pos2 = stoi(spos2);
      strand = sstrand.at(0);
    } catch (...) {
      cerr << "conversion to int failed in GenomicRegion constructor. Inputs were " << schr << " " << spos1 << " " << spos2 << endl;
      exit(EXIT_FAILURE);
    }
    return;
  }
