...
//#define r_strand(b) ((b)->core.flag&BAM_FREVERSE ? '-' : '+')
//#define r_mstrand(b) ((b)->core.flag&FMREVERSE ? '-' : '+')
#define r_endpos(b) (bam_endpos((b).get()))

//hts_itr = 0 is for whole genome
#define GET_READ(x) void* dum = 0; bam1_t* b = bam_init1(); if (hts_itr == 0)  { if (bam_read1(fp, b) < 0) { bam_destroy1(b); break; } } else if (hts_itr_next(fp, hts_itr, b, dum) <= 0) { bam_destroy1(b); break; } x = std::shared_ptr<bam1_t> (b, free_delete());
